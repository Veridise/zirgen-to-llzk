...
public:
  ...
  virtual Operation *apply(Value, ConversionPatternRewriter &, Location) const = 0;
};

class PrimitiveToRoot : public Transform {
public:
  /// Creates a constructor call to the root component ignoring the input value.
  Operation *apply(Value, ConversionPatternRewriter &rewriter, Location loc) const override {
  ...
  PrimitiveToRoot(SymbolTableCollection &st, Operation *stRoot) : st(st), stRoot(stRoot) {}
...
public:
  ...
  Operation *apply(Value from, ConversionPatternRewriter &rewriter, Location loc) const override {
...
class TransformForcedCasts : public OpConversionPattern<UnrealizedConversionCastOp> {
public:
  using OpConversionPattern<UnrealizedConversionCastOp>::OpConversionPattern;

  LogicalResult matchAndRewrite(
      UnrealizedConversionCastOp op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter
  ) const override {
  ...
  inline bool isRootComponent(Value v) const { return isRootComponent(v.getType()); }

  inline bool isVal(Type t) const { return isa<ValType>(t); }
  inline bool isVal(Value v) const { return isVal(v.getType()); }

  LogicalResult canCoerceToFinalValue(Type from, Type goal, CoercionCheckCtx &ctx) const {
    if (!from || !goal) {
    ...
    if (isComponent(from)) {
      ...
      if (isRootComponent(from)) {
        ...
        ctx.transforms.push_back(ReadSuperField::create(*superType, comp.getSuperFieldName()));
...
bool doesntHaveForcedConversion(mlir::UnrealizedConversionCastOp op) {
  ...
  if (!mlir::isa<ComponentType>(inType) || mlir::cast<ComponentType>(inType).isRoot()) {
    return true;
  }
...
class ExpandSuperCoercionPass : public ExpandSuperCoercionBase<ExpandSuperCoercionPass> {
  ...
  void runOnOperation() override {
    ...
    target.addDynamicallyLegalOp<mlir::UnrealizedConversionCastOp>(doesntHaveForcedConversion);
    target.addLegalDialect<
        ZmirDialect, mlir::func::FuncDialect, index::IndexDialect, scf::SCFDialect>();
...
std::unique_ptr<OperationPass<Zmir::ComponentOp>> createExpandSuperCoercionPass() {
